...
export class FormStore<T extends IBaseEntity<T>> {
  ...
  setCallbacks(callbacks: { onSuccess?: (result: T) => void; onError?: (error: Error) => void }) {
  ...
  hasChanges() {
    ...
    return Object.keys(this.state.data).some((key) => this.state.data?.[key as keyof T] !== this.state.originalData?.[key as keyof T]);
  }
